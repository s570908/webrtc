  ...
  socket.on("offerOrAnswer", (data) => {
    ...
    for (const [socketID, socket] of connectedPeers.entries()) {
      ...
      if (socketID !== data.socketID) {
        log(
          `Signaling Server sent offerOrAnswer to ${socketID} with data.payload.type: ${data.payload.type}`
        );
        console.log(
          `Signaling Server sent offerOrAnswer to ${socketID} with data.payload.type: ${data.payload.type}`
        );
  ...
  socket.on("candidate", (data) => {
    ...
    for (const [socketID, socket] of connectedPeers.entries()) {
      ...
      if (socketID !== data.socketID) {
        ...
        console.log(
          `Signaling Server sent candidate to ${socketID} with data.payload: ${data.payload}`
        );
